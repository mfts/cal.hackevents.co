...
def get_hh_parse_as_ical
	...
	results.each do |r|
		...
		event.description = "Hackevents: #{event_name} in #{event_city}."
...
def get_hh_events_as_ical(all_day=false)
	cal = Icalendar::Calendar.new
	cal.prodid = "-//Hackevents//cal.hackevents.co//EN"
	cal.dtstamp = Date.new
	html = HTTParty.get("http://hackevents.co").body
	doc = Nokogiri::HTML(html)
	# timezone = TZInfo::Country.get(country_code).zone_identifiers.first.to_s

	doc.css('.accordion-title').each do |e|
		...
		event = Icalendar::Event.new
		event.summary = event_name
		event.description = "Hackevents: #{event_name} hackathon in #{event_location}."
		event.location = event_location
		event.url = event_url
		event.url.ical_params = { "VALUE" => "URI" }
